...
render_proc :: proc(ctx : ^engine.Context) {
    ...
    for n : i32 = 0; n < data.cmd_lists_count; n += 1 {
        ...
        gl.BindBuffer(ctx.imgui_state.vbo_handle);
        gl.BufferData(gl.BufferTargets.Array, i32(imgui.draw_list_get_vertex_buffer_size(list) * size_of(imgui.DrawVert)), imgui.draw_list_get_vertex_ptr(list, 0), gl.BufferDataUsage.StreamDraw);

        gl.BindBuffer(ctx.imgui_state.ebo_handle);
        gl.BufferData(gl.BufferTargets.ElementArray, i32(imgui.draw_list_get_index_buffer_size(list) * size_of(imgui.DrawIdx)), imgui.draw_list_get_index_ptr(list, 0), gl.BufferDataUsage.StreamDraw);

        for j : i32 = 0; j < imgui.draw_list_get_cmd_size(list); j += 1 {
            cmd := imgui.draw_list_get_cmd_ptr(list, j);
            gl.BindTexture(gl.TextureTargets.Texture2D, gl.Texture(uint(cmd.texture_id)));
            ...
            gl.DrawElements(gl.DrawModes.Triangles, i32(cmd.elem_count), gl.DrawElementsType.UShort, idxBufferOffset);
